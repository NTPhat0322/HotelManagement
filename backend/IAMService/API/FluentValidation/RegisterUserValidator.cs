...
namespace API.FluentValidation
{
    public class RegisterUserValidator : AbstractValidator<RegisterRequest>
    {
        public RegisterUserValidator()
        {
            ...
            //RuleFor(r => r.Address)
            //    .GreaterThan(0).When(r => r.Address.HasValue) //address khác null thì mới 
            //                                                        //thực hiện so sánh giá trị
            //    .WithMessage("Address number must be greater than 0");
            RuleFor(r => r.Address)
                .MaximumLength(200).When(r => !string.IsNullOrEmpty(r.Address))
                .WithMessage("Address must not exceed 200 characters");
            RuleFor(r => r.Ward)
                .MaximumLength(100).When(r => !string.IsNullOrEmpty(r.Ward))
